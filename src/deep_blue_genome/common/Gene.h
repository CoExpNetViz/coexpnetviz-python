...
public:
	...
	Gene(GeneId, GeneCollectionId, OrthologGroupId, const std::string& name);

	GeneId get_id() const;
	GeneCollectionId get_gene_collection_id() const;
	bool has_orthologs() const;
	OrthologGroupId get_ortholog_group_id() const; // TODO assert(has)
...
private:
	...
	OrthologGroupId ortholog_group_id; // genes collection which this gene is part of
