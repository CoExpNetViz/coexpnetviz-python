...
GeneMapping::GeneMapping(string path)
{
	...
	read_file(path, [this](const char* begin, const char* end) {
		...
		auto on_line = [this](const std::vector<std::string>& line) {
			...
			to_lower(gene_name); // TODO for canonical mappings we want just case-insensitive -> canonical+, for other mappings we want: canonical -> canonical+. Might need to split
