...
def write_cytoscape(network, name):
    ...
    with open('coexpnetviz_style.xml', 'wb') as f:
        f.write(resource_string('deep_genome.coexpnetviz', 'data/coexpnetviz_style.xml'))

#TODO add to changelog: family node label is now family name instead of its correlating genes 
def _write_node_attr(network, network_name):
    ...
    nodes['correlating_genes_in_family'] = family_nodes['genes'].apply(lambda genes: ', '.join(x.name for x in genes))
